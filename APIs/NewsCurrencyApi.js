...
const NewsCurrencyApi = () => {
  ...
  useEffect(() => {
    ...
    fetch('https://bitcoin-news1.p.rapidapi.com/news', options)
  ...
  <ScrollView style={[styles.mainCategoryContainer, {backgroundColor:theme.background}]}
    ...
    <View style={{padding:8, marginBottom:224, backgroundColor:theme.background}}>
      <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity style={[styles.newsStyle, {borderColor:theme.text, backgroundColor:theme.card}]} onPress={() => {Linking.openURL(item.url);}}>
            ...
            <View style={{alignItems:'center'}}>
              <Text onPress={() => {Linking.openURL(item.url);}} style={{color:theme.text}}>To The News Page</Text>
...
export default NewsCurrencyApi