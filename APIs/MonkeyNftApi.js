...
const MonkeyNftApi = () => {
  ...
  const [isLoading, setLoading] = useState(true);
    ...
    useEffect(() => {
      ...
      fetch('https://api.nftport.xyz/v0/nfts/0xbc4ca0eda7647a8ab7c2061c2e118a18a936f13d?chain=ethereum&page_number=1&page_size=50&include=metadata&refresh_metadata=false', options)
        ...
        <FlatList
          ...
          <View>
            ...
            <View  style={[styles.touchableContainer, {backgroundColor:theme.color}]} >
              <Text  style={styles.contractName}>{item.chain}</Text>
              <Text style={[styles.contractAddress, { color:theme.background}]}>Token ID : {item.token_id}{"\n"}Contract Address :  {item.contract_address}</Text>
            </View>
          </View>
            )}
          />
